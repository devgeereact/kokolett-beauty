...
**P3**
- [ ] Shared `DataTable`/`Tooltip`/`Dropdown`/`Tabs` UI primitives — **not being built tonight, by design.** The doc's own framing already calls this "cosmetic/DX debt, not a functional gap"; a cross-cutting refactor touching every page that hand-rolls its own table/dropdown is real scope, not a same-night fix, and nothing about it is broken for the owner today. (`Timeline` is now its own component — `CustomerTimeline.tsx`, see Customers section above.)
- [ ] Product-event/analytics instrumentation — **not being built, by design.** Deliberately low priority for a single-owner product (the doc's own reasoning: an owner glancing at Reports covers most of what a funnel would show). Adding tracking infrastructure nobody asked for is scope invention, not a gap-fill, and carries its own privacy/GDPR surface for a UK small business — not a call to make unilaterally overnight.
