...
const TONE_ROTATION: Tone[] = ['primary', 'pending', 'in_service', 'confirmed', 'urgent', 'completed'];

function toneForCategory(name: string): Tone {
  const known = CATEGORY_TONES[name];
  if (known) return known;
  let hash = 0;
  for (let i = 0; i < name.length; i += 1) hash = (hash + name.charCodeAt(i)) % TONE_ROTATION.length;
  return TONE_ROTATION[hash]!;
}

const THUMB_PX: Record<'sm' | 'md' | 'lg', number> = { sm: 32, md: 40, lg: 48 };
const THUMB_CLASS: Record<'sm' | 'md' | 'lg', string> = { sm: 'h-8 w-8', md: 'h-10 w-10', lg: 'h-12 w-12' };

/** A real style photo when one's been uploaded, the same tinted placeholder as everywhere else when not. */
function ServiceThumb({ item, size }: { item: ServiceMenuItem; size: 'sm' | 'md' | 'lg' }): JSX.Element {
  if (!item.image_path) return <Avatar name={item.name} size={size} />;
  const px = THUMB_PX[size];
  return (
    <img
      src={buildImageKitUrl(item.image_path, { width: px * 2, height: px * 2, crop: 'maintain_ratio' })}
      alt=""
 ...
 */
export const ServicesCatalogue = forwardRef<ServicesCatalogueHandle>(function ServicesCatalogue(
  _props,
  ref,
): JSX.Element {
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {s.note && (
                      <p className="mt-0.5 line-clamp-2 text-sm text-muted-foreground">{s.note}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between border-t border-border pt-1.5 text-sm">
                  ...
                  <div>
                    ...
                    <p className="flex items-center gap-1 font-medium text-foreground">
                      <Timer aria-hidden="true" className="h-3.5 w-3.5 shrink-0" strokeWidth={2} />
