...
export function BookingRulesCard({ settings }: { settings: BookingSettings | null }): JSX.Element {
  const rows = [
    { label: 'Advance booking window', value: settings ? `${settings.max_horizon_days} days` : '—' },
    {
      label: 'Minimum notice',
      value: settings ? `${Math.round((settings.lead_time_min / 60) * 10) / 10} hours` : '—',
    },
    {
      label: 'New customer window',
      value: settings ? `${settings.approval_window_h} hours` : '—',
    },
    { label: 'Buffer time between bookings', value: settings ? `${settings.default_buffer_min} minutes` : '—' },
    { label: 'Max appointments per day', value: settings ? String(settings.max_appointments_per_day) : '—' },
  ];
        ...
        {rows.map((r) => (
          <div key={r.label} className="flex items-center justify-between gap-3 text-sm">
            <dt className="flex items-center gap-2 text-muted-foreground">
              <CalendarClock aria-hidden="true" className="h-4 w-4 shrink-0" strokeWidth={2} />
              {r.label}
            </dt>
